...
export default class LoginPage extends Component {
        ...
        return (
            <View style={styles.container}>
                <View style={styles.welcomeStyle}>
                    <Text style={{fontSize: 24}}>Welcome to Fetcher</Text>
                </View>
    ...
    login() {
        if (JSON.parse(this.recvMsg).state === 0) {
            // 登陆成功
            // 记录登录
            console.warn(JSON.parse(this.recvMsg));
            storage.save({
                key: 'hasLogined',
                data: {
            ...
            ToastAndroid.show('意料外的错误，联系管理员', ToastAndroid.SHORT);
        }
    }
